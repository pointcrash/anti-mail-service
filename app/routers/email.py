...
async def send_email(request: EmailRequest, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    # Create DB entry
...
async def get_status(email_id: int, db: AsyncSession = Depends(get_db)):
